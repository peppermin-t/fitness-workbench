(function () {
    ...
    function normalizeState(input, defaults) {
        ...
        if (!next.gyms.length && defaults?.gyms?.length)
            next.gyms = clone(defaults.gyms);
    ...
    function relinkSessionExerciseLogs(sessions, exerciseLogs) {
        ...
        exerciseLogs.forEach((log) => {
            if (!log.sessionId || !byId.has(log.sessionId))
                return;
            const session = byId.get(log.sessionId);
            if (log.id && !session.exerciseLogIds.includes(log.id))
    ...
    function buildStats(log, previousLogs) {
        ...
        if (!stats)
            return { volumeLoad: 0, hardSets: 0, simplePr: { isPr: false, records: [] } };
