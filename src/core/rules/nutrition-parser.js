...
(function () {
    ...
    function parseNutritionLog(rawText, goal, latestMetric) {
        ...
        const mealResults = meals.map((meal) => {
            ...
            items.forEach((item) => {
                ...
                if (item.uncertain)
                    mealTags.add("portion_uncertain");
            });
            ...
            if (mealTags.has("portion_uncertain"))
                missingInfo.push(mealMissingPrompt(meal.meal, items));
            return {
                meal: meal.meal,
                text: meal.text,
                items,
    ...
    function splitMeals(text) {
        ...
        for (const part of text.split(/[，。；;]/).map((x) => x.trim()).filter(Boolean)) {
            ...
            if (found) {
                if (buffer)
                    pieces.push({ meal: currentMeal, text: buffer });
                currentMeal = found[0];
                buffer = part;
            }
            else {
    ...
    function nutritionTagsFor(text, items, totals, mealName) {
        const tags = [];
        const add = (tag) => { if (!tags.includes(tag))
            tags.push(tag); };
        if (/没吃|不吃|没饭|没吃饭/.test(text))
            add("missed_meal");
        if ((items || []).length === 0)
            add("unparsed_food");
        if ((items || []).some((item) => item.tags.includes("processed_snack")))
            add("processed_snack");
        if ((items || []).some((item) => item.tags.includes("high_fat_possible")))
            add("high_fat_possible");
        if ((items || []).some((item) => item.tags.includes("high_sodium_possible")))
            add("high_sodium_possible");
        if ((items || []).some((item) => item.tags.includes("high_sugar_possible")))
            add("high_sugar_possible");
        if ((items || []).some((item) => item.tags.includes("fruit_present")))
            add("fruit_present");
        if ((items || []).some((item) => item.tags.includes("vegetable_present")))
            add("vegetable_present");
        if ((items || []).some((item) => item.tags.includes("portion_uncertain")) || /一点|一些|少量|大概|火锅|外卖|随便/.test(text))
            add("portion_uncertain");
        if (totals.protein >= 25)
            add("high_protein");
        else if (mealName !== "snack" && mealName !== "all_day")
            add("low_protein_possible");
        if (totals.carbs >= 25)
            add("carb_present");
        if (totals.fiber < 4 && !/(菜|蔬|水果|苹果|香蕉|西兰花|青菜|沙拉)/.test(text))
            add("low_fiber_possible");
        if (totals.calories >= 500)
            add("high_calorie_meal");
        return tags;
    }
    ...
    function evaluateNutritionDay({ totals, goal, target, mealDistribution }) {
        const tags = [];
        const primary = goal?.primaryGoal || "general_fitness";
        if (totals.protein < target.protein * 0.7)
            tags.push("daily_protein_gap");
        if (totals.fiber < target.fiber * 0.7)
            tags.push("daily_fiber_gap");
        if (totals.calories < target.caloriesLower * 0.75)
            tags.push("daily_energy_low");
        if (totals.calories > target.caloriesUpper * 1.1)
            tags.push("daily_energy_high");
        if ((primary === "strength" || primary === "muscle_gain") && totals.carbs < target.carbsLower * 0.75)
            tags.push("daily_carb_low");
        if (mealDistribution.highestProteinShare >= 0.6)
            tags.push("protein_distribution_unbalanced");
    ...
    function nutritionConfidence(text, meals) {
        let score = 0.78;
        if (/一点|一些|少量|大概|火锅|外卖|随便/.test(text))
            score -= 0.2;
        ...
        if (meals.flatMap((meal) => meal.items).length <= 1)
            score -= 0.08;
        if (score >= 0.75)
            return "high";
        if (score >= 0.55)
    ...
    function buildNutritionProfile(logs, goal) {
        ...
        (logs || []).forEach((log) => {
            ...
            if ((log.analysis?.tags || []).includes("daily_protein_gap"))
                lowProteinDays += 1;
            if ((log.analysis?.tags || []).includes("missed_meal"))
                missedMealDays += 1;
            if (log.analysis?.confidence !== "high")
                uncertainDays += 1;
    ...
    function inferNutritionProfileAdvice(topTags, goal, stats) {
        ...
        const primary = goal?.primaryGoal || "general_fitness";
        const notes = [];
        if (tags.includes("missed_meal"))
            notes.push("最近多次出现漏正餐或正餐质量不足，优先建立可执行的保底简餐。");
        if (tags.includes("processed_snack"))
            notes.push("加工零食出现较多，容易影响饱腹感和训练供能稳定性。");
        if (tags.includes("low_fiber_possible"))
            notes.push("蔬菜/水果/纤维记录偏少，建议先固定每天 1-2 份蔬果。");
        if (tags.includes("high_fat_possible") || tags.includes("high_sodium_possible"))
            notes.push("外食油脂和钠不确定性较高，火锅/炒面/烧烤类需要更留意份量。");
        if (stats.lowProteinDays >= Math.max(2, Math.ceil(stats.count / 3)))
            notes.push(`近 ${stats.count} 天里有 ${stats.lowProteinDays} 天蛋白估算偏低，阶段 2 要先把“白天补蛋白”固定下来。`);
        if (stats.missedMealDays >= Math.max(2, Math.ceil(stats.count / 3)))
            notes.push(`最近漏餐/正餐空缺比较频繁（${stats.missedMealDays}/${stats.count} 天），优先建立出差保底简餐模板。`);
        if (stats.uncertainDays >= Math.max(2, Math.ceil(stats.count / 2)))
            notes.push("多数饮食记录分量仍偏模糊，后续至少补几份/几碗/多少克，趋势判断会更可靠。");
        if (primary === "fat_loss")
            notes.push("减脂目标下，优先保证蛋白和正餐完整，再处理零食和高油外食。");
        if (primary === "muscle_gain")
            notes.push("增肌目标下，需要把蛋白和碳水更均匀地分布到白天各餐。");
        if (primary === "strength")
            notes.push("力量目标下，如果白天经常缺餐，训练表现和动作质量会更不稳定。");
        if (!notes.length)
            notes.push("饮食记录还不够多，继续积累数据后再看长期模式。");
        return notes;
    }
    ...
    function averageNutritionTotals(entries) {
        if (!entries.length)
            return roundNutritionTotals(emptyNutritionTotals());
    ...
    function parseFoodQuantity(quantityToken, unitToken, defaultUnit) {
        ...
        if (!quantityToken)
            return { quantity: 1, unit, uncertain: !unitToken };
        if (/一点|一些|少量|少许/.test(quantityToken))
            return { quantity: 0.5, unit, uncertain: true };
    ...
    function mealMissingPrompt(mealName, items) {
        ...
        if (!items.length)
            return `${label}描述较模糊，补充主食/肉量会更准确。`;
        if (items.some((item) => item.label.includes("火锅")))
            return `${label}是火锅，补充肉量、主食和蘸料会更准确。`;
