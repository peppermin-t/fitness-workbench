...
(function () {
    ...
    function analyzeExerciseFeedback(input, exercise) {
        ...
        const evidenceBase = [];
        if (input.rangeOfMotion)
            evidenceBase.push(`动作幅度：${input.rangeOfMotion}`);
        if (input.sideIssue && input.sideIssue !== "none")
            evidenceBase.push(`左右差异：${input.sideIssue}`);
        if (input.targetMuscleFeel)
            evidenceBase.push(`目标肌肉感觉：${input.targetMuscleFeel}`);
        if (input.limitingFactor && input.limitingFactor !== "unknown")
            evidenceBase.push(`限制因素：${input.limitingFactor}`);
        if (Number(input.rpe))
            evidenceBase.push(`RPE ${input.rpe}/10`);
        if (Number(input.painScore) >= 0)
            evidenceBase.push(`疼痛 ${input.painScore}/5`);
        if (input.freeText)
            evidenceBase.push(`原始反馈：${input.freeText}`);
    ...
    function buildExerciseProfiles(logs) {
        ...
        (logs || []).forEach((log) => {
            ...
            profile.count += 1;
            if (log.quality === "poor")
                profile.poorQualityCount += 1;
            if (Number(log.painScore) >= 3)
                profile.painCount += 1;
            if ((log.createdAt || log.date || "") > profile.latestAt)
                profile.latestAt = log.createdAt || log.date || "";
    ...
    function computeExercisePriority(topTags, profile) {
        ...
        if (profile.painCount)
            score += profile.painCount * 4;
        if (profile.poorQualityCount)
            score += profile.poorQualityCount * 2;
        topTags.forEach(([tag, count]) => {
            if (tag === "pain_risk")
                score += count * 4;
            if (tag === "grip_limiting" || tag === "poor_target_muscle_feel")
                score += count * 2;
            if (tag === "reduced_rom_late" || tag === "technique_breakdown")
                score += count * 2;
            if (tag === "left_weaker" || tag === "right_weaker")
                score += count * 2;
        });
        return score;
    }
    function inferExerciseProfileAdvice(topTags, profile) {
        ...
        if (tags.includes("pain_risk"))
            notes.push("该动作多次出现疼痛或不适，优先降强度或替换成更稳的版本。");
        if (tags.includes("reduced_rom_late"))
            notes.push("该动作后程容易半程，下次先恢复完整幅度，再考虑加重。");
        if (tags.includes("left_weaker") || tags.includes("right_weaker"))
            notes.push("该动作存在左右差异，单侧训练时以弱侧高质量完成度作为标准。");
        if (tags.includes("grip_limiting"))
            notes.push("该动作常被握力限制，背部训练可优先考虑助力带或更稳定器械。");
        if (tags.includes("poor_target_muscle_feel"))
            notes.push("该动作目标肌肉感觉偏弱，建议加入激活动作或顶峰停顿。");
        if (tags.includes("technique_breakdown"))
            notes.push("该动作历史上代偿较多，先稳住技术，再继续堆量。");
        if (!notes.length && profile.count >= 2)
            notes.push("该动作近期整体稳定，继续观察是否满足渐进超负荷条件。");
        return notes;
    }
