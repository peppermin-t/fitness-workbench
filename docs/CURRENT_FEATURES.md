...
本文档按当前代码实际实现梳理功能边界，主要依据 `README.md`、`docs/PRODUCT_VISION.md`、`index.html`、`data.js`、`src/core/rules/*`、`planner.js`、`app.js` 和 `styles.css`。当前应用是零依赖静态页面，通过 `index.html` 直接加载 `data.js`、`src/core/rules/*`、`planner.js`、`app.js`，运行数据存放在浏览器 `localStorage`。
...
- 部分渲染函数在文件中重复定义，后面的同名函数覆盖前面的实现，增加维护风险。
