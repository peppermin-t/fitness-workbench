(function () {
    ...
    function buildIntegratedSignals({ goal, metrics, sessions, exerciseLogs, nutritionLogs }) {
        ...
        if (primary === "fat_loss" && weightTrend21 && Math.abs(weightTrend21.delta) <= 0.4) {
            const likelyCauses = [];
            if (nutritionProfile.topTags.some(([tag]) => tag === "high_fat_possible" || tag === "processed_snack"))
                likelyCauses.push("高油外食或零食频率较高");
            if (nutritionProfile.missedMealDays >= 2)
                likelyCauses.push("正餐连续性差");
            if (nutritionProfile.uncertainDays >= 2)
                likelyCauses.push("饮食分量记录偏模糊");
