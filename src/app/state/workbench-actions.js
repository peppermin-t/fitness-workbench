(function () {
    ...
    function create({ rules, uid, nowLabel, todayIso }) {
        ...
        function saveExerciseLog({ state, dayIndex, day, exercise, plannedRow, input }) {
            ...
            if (!session.exerciseLogIds.includes(log.id))
                session.exerciseLogIds.push(log.id);
            moveSessionToFront(state, session.id);
            const advice = {
                id: uid("advice"),
                createdAt: nowLabel(),
        ...
        function applyReviewCandidate({ state, candidate, getExercise }) {
            ...
            if (matched?.status === "applied")
                return { status: "already_applied", revision: matched };
            ...
            if (rev.patch.type === "reduce_day_volume") {
                day.exercises.forEach((row) => {
                    if (Number(row.sets) > 1)
                        row.sets = Math.max(2, Math.round(Number(row.sets) * rev.patch.factor));
    ...
    function parseLoadKg(text) {
        if (/自重|bodyweight/i.test(String(text || "")))
            return null;
    ...
    function bumpDurationText(text, minutes) {
        ...
        if (singleMatch)
            return `${Number(singleMatch[1]) + minutes} ${singleMatch[2]}`;
