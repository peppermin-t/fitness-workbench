...
下一轮如果进入正式重构，不先改 UI，不先上框架，优先从旧集中规则入口中的纯函数拆分开始。
...
- 同时补最小测试或示例用例。
- 保持 `window.FitnessCore.Rules` 作为统一规则入口；`src/app/main.ts` 已迁移为使用 `window.FitnessCore.Rules`。
- 不改变页面结构和交互行为。

### 原因

旧集中规则入口里曾经承载目标解析、计划生成、动作反馈分析、饮食解析、画像、联动信号和周复盘等核心业务规则。先拆纯函数可以降低复杂度，并且比先改 UI 风险更低。
