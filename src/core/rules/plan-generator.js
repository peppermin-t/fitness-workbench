...
(function () {
    ...
    function planDayTypes(daysCount, goal) {
        ...
        if (daysCount === 2)
            return ["upper", "lower"];
        if (daysCount === 3)
            return goal === "fat_loss" ? ["upper", "lower", "conditioning"] : ["upper", "lower", "full"];
        if (daysCount === 4)
            return goal === "muscle_gain" ? ["push", "pull", "lower", "upper"] : ["upper", "lower", "full", "conditioning"];
        if (daysCount === 5)
            return ["push", "pull", "lower", "upper", "conditioning"];
        return ["push", "pull", "lower", "upper", "full", "conditioning"];
    }
    function planConfig(goal) {
        if (goal.primaryGoal === "strength")
            return { sets: 4, reps: "3-6", rpe: "7-8", rest: "2-3 分钟", cardio: "15-20 分钟" };
        if (goal.primaryGoal === "muscle_gain")
            return { sets: 3, reps: "8-12", rpe: "7-8", rest: "90-120 秒", cardio: "10-15 分钟" };
        if (goal.primaryGoal === "fat_loss")
            return { sets: 3, reps: "10-15", rpe: "6-8", rest: "60-90 秒", cardio: "20-30 分钟" };
        return { sets: 3, reps: "8-12", rpe: "6-7", rest: "90 秒", cardio: "15-20 分钟" };
    }
    function defaultRowNote(goal, pattern) {
        if (goal.primaryGoal === "fat_loss" && pattern === "有氧")
            return "保持可持续强度，优先保证训练连续性。";
        if (goal.secondaryGoal === "strength_maintenance")
            return "保留动作质量，不用每次都冲重量。";
        if (goal.frequentTravel)
            return "出差模式：优先可执行和低录入成本。";
    ...
    function buildPlanContext(gym, goal, parsed, metrics) {
        ...
        const notes = [];
        if (parsed.frequentTravel)
            notes.push("出差优先：每次训练控制在可执行范围");
        if (gym.equipment.length <= 5)
            notes.push("器械受限：启用替代动作优先级");
        if (latest) {
            if (parsed.primaryGoal === "fat_loss" && trend?.delta > 0.3)
                notes.push("减脂目标下体重近30天上升，建议增加有氧/检查饮食");
            if (parsed.primaryGoal === "muscle_gain" && trend && Math.abs(trend.delta) < 0.3)
                notes.push("增肌目标下体重变化小，关注热量摄入和渐进超负荷");
            if (parsed.primaryGoal === "fat_loss" && latest.skeletalMuscle)
                notes.push("减脂期保留力量训练，关注骨骼肌变化");
        }
        return {
